    ...
    const GetRating = async () => {
        ...
        try {
            const response = await axios(
                "https://sinaharaeeni.ir/DoctorPanel/get_rating/",
                {
                    method: "GET",
                    headers: {
                            ...
                            <MDBCard className="mb-5" style={{ borderRadius: '15px' }}>
                                <MDBCardBody className="p-4">
                                    ...
                                    <div style={{ justifyContent: "center", textAlign: "center" }}>
                                        <hr style={{color: "#8A7DFA", opacity: "0.45"}}className="my-4" />
                                        <div className="d-flex justify-content-center align-items-center">
                                            <Rating size="large" name="read-only" precision={0.5} value={rate} readOnly />
                                        </div>
                                        <div style={{marginTop: "2%"}}className="d-flex justify-content-center align-items-center mb-3">
                                            <MDBCardText style= {{fontSize: "20px"}}className="text-uppercase mb-0">
                                                <MDBIcon fas icon="cog me-2" />.تا کنون {rateCount ? convertToPersianNumbers(rateCount) : <></>} نفر به شما امتیاز داده اند
                                            </MDBCardText>
                                        </div>
                                        <div style={{marginTop: "5%"}}>
                                        <a style={{color: "#B9B1FD"}}href="#" onClick={(e) => navigate("/Home")}>
                                            {" "}
                                            بازگشت به صفحۀ اصلی
                                            </a>
                                        </div>
